...
var HelloServiceClient = function () {
  function HelloServiceClient(rs, tracer, meterRegistry) {
    this._rs = rs;
    this._tracer = tracer;
    this.sayHelloTrace = rsocket_rpc_tracing.traceSingle(tracer, "HelloService", {"rsocket.rpc.service": "jsug.HelloService"}, {"method": "sayHello"}, {"rsocket.rpc.role": "client"});
    this.sayHelloMetrics = rsocket_rpc_metrics.timedSingle(meterRegistry, "HelloService", {"service": "jsug.HelloService"}, {"method": "sayHello"}, {"role": "client"});
    this.lotsOfRepliesTrace = rsocket_rpc_tracing.trace(tracer, "HelloService", {"rsocket.rpc.service": "jsug.HelloService"}, {"method": "lotsOfReplies"}, {"rsocket.rpc.role": "client"});
    this.lotsOfRepliesMetrics = rsocket_rpc_metrics.timed(meterRegistry, "HelloService", {"service": "jsug.HelloService"}, {"method": "lotsOfReplies"}, {"role": "client"});
  }
  HelloServiceClient.prototype.sayHello = function sayHello(message, metadata) {
    ...
    return this.sayHelloMetrics(
      this.sayHelloTrace(map)(new rsocket_flowable.Single(subscriber => {
        ...
        var metadataBuf = rsocket_rpc_frames.encodeMetadata('jsug.HelloService', 'SayHello', tracingMetadata, metadata || Buffer.alloc(0));
  ...
  HelloServiceClient.prototype.lotsOfReplies = function lotsOfReplies(message, metadata) {
    ...
    return this.lotsOfRepliesMetrics(
      this.lotsOfRepliesTrace(map)(new rsocket_flowable.Flowable(subscriber => {
        var dataBuf = Buffer.from(message.serializeBinary());
        var tracingMetadata = rsocket_rpc_tracing.mapToBuffer(map);
        var metadataBuf = rsocket_rpc_frames.encodeMetadata('jsug.HelloService', 'LotsOfReplies', tracingMetadata, metadata || Buffer.alloc(0));
...
var HelloServiceServer = function () {
  function HelloServiceServer(service, tracer, meterRegistry) {
    ...
    this._tracer = tracer;
    this.sayHelloTrace = rsocket_rpc_tracing.traceSingleAsChild(tracer, "HelloService", {"rsocket.rpc.service": "jsug.HelloService"}, {"method": "sayHello"}, {"rsocket.rpc.role": "server"});
    this.sayHelloMetrics = rsocket_rpc_metrics.timedSingle(meterRegistry, "HelloService", {"service": "jsug.HelloService"}, {"method": "sayHello"}, {"role": "server"});
    this.lotsOfRepliesTrace = rsocket_rpc_tracing.traceAsChild(tracer, "HelloService", {"rsocket.rpc.service": "jsug.HelloService"}, {"method": "lotsOfReplies"}, {"rsocket.rpc.role": "server"});
    this.lotsOfRepliesMetrics = rsocket_rpc_metrics.timed(meterRegistry, "HelloService", {"service": "jsug.HelloService"}, {"method": "lotsOfReplies"}, {"role": "server"});
